...
GEOPOLITICS_DEFAULT_FETCH_QUERY = "Iran Ukraine Russia China sanctions war Federal Reserve"
...
class BriefService:
    ...
    async def run(
        ...
        # When no user query is given and context is geopolitics, use a default search query
        # so that Yahoo Finance returns globally relevant geopolitical news rather than only
        # the latest US financial headlines.
        if normalized_context == CONTEXT_GEOPOLITICS and not normalized_query:
            fetch_query: str | None = GEOPOLITICS_DEFAULT_FETCH_QUERY
        else:
            fetch_query = normalized_query

        cache_key = self._cache_key(
            continents=fetch_continents,
            window_hours=window_hours,
            geo_focus=normalized_geo_focus,
            custom_query=fetch_query,
